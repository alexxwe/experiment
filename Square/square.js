...
function addPositionsForSide(side, x, y) {
    for (let p = 0; p < squarePlacements.length - 1; p++) {
        const offset = side == 'top' || side == 'right' ? pathSize * squarePlacements[p] : pathSize - pathSize * squarePlacements[p]
        const start = side == 'top' || side == 'bottom' ? x : y
        const positionStart = start + offset
        const step = (pathSize * squarePlacements[p + 1] - pathSize * squarePlacements[p]) / positionsPerStep
        for (let i = 0; i < positionsPerStep; i++) {
            const adjustment = side == 'top' || side == 'right' ? step * i : -step * i
            path.push({
...
class Square {
    ...
    draw() {
        ...
        square(path[this.position].x - this.size / 2, path[this.position].y - this.size / 2, this.size)
    }
}

function createSquares() {
    for (let i = 0; i < path.length; i += positionsPerStep) {
        squares.push(new Square(i, squareSize, color(Math.floor((i / path.length) * 360), 100, 34)))
    }
}
